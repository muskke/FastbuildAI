 ...
 * 拼接文件URL地址
 *
 * @param filePath 文件相对路径，例如 /uploads/aaa/1.jpg 或 uploads/aaa/1.jpg
 * @param domain 域名，例如 https://example.com，如果不提供则使用环境变量中的域名
 * @param options 选项
 * @returns 完整的文件URL地址
 */
export function getFileUrl(
    ...
    domain?: string,
    options?: {
        /** 是否强制使用HTTPS协议，默认为false */
        forceHttps?: boolean;
        /** 环境变量中域名的键名，默认为 NUXT_PUBLIC_API_BASE_URL */
        domainEnvKey?: string;
        /** 是否添加时间戳防止缓存，默认为false */
        noCache?: boolean;
    },
): string {
    if (!filePath) return "";
    ...
    const defaultOptions = {
        forceHttps: false,
        domainEnvKey: "VITE_APP_BASE_URL",
        noCache: false,
    };
    ...
    if (!finalDomain) {
        // 如果没有提供域名，则从环境变量中获取
        finalDomain = process.env[mergedOptions.domainEnvKey] || "";
    }

    // 如果强制使用HTTPS协议，则将HTTP替换为HTTPS
    if (mergedOptions.forceHttps && finalDomain.startsWith("http:")) {
        finalDomain = finalDomain.replace(/^http:/i, "https:");
    }
    ...
    // 拼接域名和文件路径
    let url = finalDomain
        ? `${finalDomain.replace(/\/$/, "")}${normalizedFilePath}`
        : normalizedFilePath;
...
/**
 * 拼接文件URL地址（使用ConfigService）
 *
 * @param filePath 文件相对路径，例如 /uploads/aaa/1.jpg 或 uploads/aaa/1.jpg
 * @param configService ConfigService实例，用于获取配置
 * @param options 选项
 * @returns 完整的文件URL地址
 */
export function getFileUrlWithConfig(
    filePath: string,
    configService: ConfigService,
    options?: {
        /** 是否强制使用HTTPS协议，默认为false */
        forceHttps?: boolean;
        /** 配置中域名的键名，默认为 app.apiBaseUrl */
    ...
    const defaultOptions = {
        forceHttps: false,
        domainConfigKey: "app.apiBaseUrl",
        noCache: false,
    };
    ...
    // 从配置服务中获取域名
    const domain = configService.get<string>(mergedOptions.domainConfigKey) || "";

    return getFileUrl(filePath, domain, {
        forceHttps: mergedOptions.forceHttps,
