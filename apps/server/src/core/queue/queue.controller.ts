...
export class QueueController {
    ...
    async getJobById(@Param("queueName") queueName: string, @Param("jobId") jobId: string) {
        ...
        if (!job) {
            return { success: false, message: `任务不存在: ${queueName}/${jobId}` };
        }
    ...
    async removeJob(@Param("queueName") queueName: string, @Param("jobId") jobId: string) {
        ...
        if (result) {
            return { success: true, message: `任务已删除: ${queueName}/${jobId}` };
        } else {
            return { success: false, message: `任务不存在或删除失败: ${queueName}/${jobId}` };
        }
    }
    ...
    async retryJob(@Param("queueName") queueName: string, @Param("jobId") jobId: string) {
        ...
        if (result) {
            return { success: true, message: `任务已重试: ${queueName}/${jobId}` };
        } else {
            return { success: false, message: `任务不存在或重试失败: ${queueName}/${jobId}` };
        }
    }
}
