...
import { TEAM_ROLE } from "../constants/team-role.constants";
import { TEAM_ROLE_PERMISSIONS } from "../constants/team-role.constants";
...
export class DatasetPermissionGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (!datasetId) {
            throw HttpExceptionFactory.badRequest(`缺少参数: ${datasetIdParam}`);
        }
        ...
        try {
            ...
            throw HttpExceptionFactory.forbidden("没有权限执行此操作");
        }
    }
    ...
    private async checkResourceOwnership(
        ...
        if (userRole === TEAM_ROLE.EDITOR) {
            const resourceId = this.getResourceIdFromRequest(request, resourceType);
            if (!resourceId) {
