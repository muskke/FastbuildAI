...
export class PermissionController extends BaseController {
     ...
     * 获取权限列表
     *
     * @param type 权限类型，system表示系统权限，plugin表示插件权限
    ...
    async findAll(
        ...
        if (typeParam) {
            ...
            } else if (typeParam === "plugin") {
                baseCondition.type = PermissionType.PLUGIN;
            }
        }
        ...
        if (group) {
            ...
            const groupCondition = { ...baseConditionCopy, group: ILike(`%${group}%`) };
            const groupNameCondition = { ...baseConditionCopy, groupName: ILike(`%${group}%`) };
        ...
        if (keyword) {
            ...
            const nameCondition = { ...baseCondition, name: ILike(`%${keyword}%`) };
            const codeCondition = { ...baseCondition, code: ILike(`%${keyword}%`) };
