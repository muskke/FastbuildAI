...
} from "@/services/console/permission";

import PopEdit from "./edit.vue";
...
const columnLabels = computed<Record<string, string>>(() => ({
    // select: t("console-system-perms.permission.select"),
    code: t("console-system-perms.permission.code"),
    name: t("console-system-perms.permission.name"),
    description: t("console-system-perms.permission.describe"),
    group: t("console-system-perms.permission.group"),
    isDeprecated: t("console-system-perms.permission.isDeprecated"),
...
function getRowItems(row: Row<TreePermission>) {
    return [
        hasAccessByCodes(["permission:info"])
            ? {
                  label: t("console-common.view"),
                  icon: "i-lucide-eye",
                  onSelect() {
                      currentCode.value = row.original.code;
...
const { lockFn: syncPermissions, isLock: syncLoading } = useLockFn(async () => {
    try {
        ...
        toast.success(
            t("console-system-perms.permission.syncSuccess", {
                added: result.added,
...
const { lockFn: cleanupPermissions, isLock: cleanupLoading } = useLockFn(async () => {
    try {
        ...
        toast.success(
            t("console-system-perms.permission.cleanupSuccess", {
    ...
    <div class="permission-list-container pb-5">
        ...
        <div class="bg-background sticky top-0 z-10 flex flex-wrap gap-4 pb-4">
            <UInput
                ...
                :placeholder="t('console-system-perms.permission.nameInput')"
                @change="getPermissions"
            />

            <UInput
                v-model="searchForm.group"
                :placeholder="t('console-system-perms.permission.groupInput')"
            ...
            <div class="flex items-end gap-2 md:ml-auto">
                <AccessControl :codes="['permission:sync']">
                    <UButton
                        color="info"
                        variant="outline"
                        :label="t('console-system-perms.permission.sync')"
                ...
                <AccessControl :codes="['permission:cleanup']">
                    <UButton
                        color="warning"
                        variant="outline"
                        :label="t('console-system-perms.permission.cleanup')"
                ...
                <AccessControl :codes="['permission:list']">
                    <UButton
                        ...
                        :loading="listLoading"
                    >
                        {{ t("console-system-perms.permission.refresh") }}
                    </UButton>
                </AccessControl>
            </div>
        </div>
        ...
        <div class="border-default mt-auto flex items-center justify-between gap-3 border-t pt-4">
            <div class="text-muted text-sm">
                {{ t("console-system-perms.permission.totalCount", { count: permissions.length }) }}
            </div>
            ...
            <div class="flex items-center gap-1.5">
                <UButton
                    ...
                    color="gray"
                    variant="ghost"
                    :label="t('console-system-perms.permission.groupView')"
        ...
        <!-- 弹窗详情组件 -->
        <PopEdit v-if="showPopDetail" :code="currentCode" @close="handlePopClose" />
    </div>
</template>
